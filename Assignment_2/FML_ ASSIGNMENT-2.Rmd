...
# Data cleaning
```{r}
Universal.df <- read.csv("C:/Users/micha/OneDrive/Desktop/FML/UniversalBank.csv")
```
...
# changing to factor(Normalization)
```{r}
Universal.df$Education <- as.factor(Universal.df$Education)
```
# Dummy variables(one heart encoding)
```{r}
...
```
# to select the paticular data to run
```{r}
set.seed(1)
# Selected rows
train.index <- sample(row.names(universal_m.df),0.6*dim(universal_m.df)[1])
# data other than selected data
valid.index <- setdiff(rownames(universal_m.df),train.index)
...
## to calculate the mean and standard devation(Standadization)
